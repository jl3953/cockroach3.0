...
/*func TestDemoteSingleHotkey(t *testing.T) {
	...
	stopper := stop.NewStopper()
	defer stopper.Stop(ctx)
	store, _ := createTestStore(t, testStoreOpts{
		createSystemRanges: true,
	}, stopper)
	_ = store.Start(ctx, stopper)

	//txn := kv.NewTxn(ctx, store.DB(), store.nodeDesc.NodeID)
	...
	firstTime := true
	for i := 0; i < 1; i++ {
		if rangeId, err = store.AllocateRangeID(ctx); err != nil {
			log.Fatal(ctx, "jenndebug store.AllocateRangeID failed on the %d iteration %+v\n",
				i, err)
		}

		replicaId := roachpb.ReplicaID(i+5)
		if repl, created, err := store.getOrCreateReplica(ctx, rangeId, replicaId, &roachpb.ReplicaDescriptor{
			NodeID:    store.nodeDesc.NodeID,
			StoreID:   store.StoreID(),
			ReplicaID: replicaId,
		}, false); !created {
			log.Fatal(ctx, "jenndebug getOrCreateReplica not created on rangeId %+v replicaId %+v\n",
				rangeId, replicaId)
		} else if err != nil {
			log.Fatalf(ctx, "jenndebug getOrCreateRelica err %+v rangeId %+v replicaId %+v created %b\n",
				err, rangeId, replicaId, created)
		} else {
			repl.raftMu.Unlock()
		}

		if firstTime {
			_ = WriteRandomDataToRange(t, store, rangeId, key)
			firstTime = false
		}
	}
	repl := store.LookupReplica(key)
	...
	if err := repl.DemoteSingleHotkey(ctx, timestamp, key, &value); err != nil {
		log.Warningf(ctx, "jenndebug err %+v\n", err)
	}
}*/
