...
func mergeCRDBCicadaResponses(ctx context.Context,
	...
	for i, promotedToCicada := range isInCicada {
		if promotedToCicada {
			...
			} else if cputReq := originalRequests[i].GetConditionalPut(); cputReq != nil {
				br.Responses[i].Value = &roachpb.ResponseUnion_ConditionalPut{
					ConditionalPut: &roachpb.ConditionalPutResponse{},
				}
			} else if iputReq := originalRequests[i].GetInitPut(); iputReq != nil {
				br.Responses[i].Value = &roachpb.ResponseUnion_InitPut{
					InitPut: &roachpb.InitPutResponse{},
				}
			} else {
...
func (txn *Txn) Send(
	...
	for i, req := range originalRequests {
		...
		//log.Warningf(ctx, "jenndebug any req key %+v, txn.Id %d\n", req.GetInner().Header().Key, txn.ID())
		if key := req.GetInner().Header().Key; IsUserKey(key.String()) && len(originalRequests) <= 2 {
			if cicadaAffiliatedKey, isPromoted := txn.DB().IsKeyInCicadaAtTimestamp(
				...
				if putReq := req.GetPut(); putReq != nil {
					...
					if tableName, _ := txn.DB().TableName(ExtractTableNum(key)); ORDER == tableName {
						warmKeysRequests = warmKeysRequests[:len(warmKeysRequests)-1]
						brSelfBuffer = &roachpb.BatchResponse{
							Responses: make([]roachpb.ResponseUnion, 1),
						}
...
func reconstructValue(key []byte, value []byte, walltime int64, logicaltime int32, isZero []byte) [][]byte {
	...
	if hasHeader(value, isZero) {
		...
		zeroCorrectedVal := roachpb.Value{
			RawBytes:  make([]byte, len(value)),
			Timestamp: hlc.Timestamp{},
		}
		for j, b := range value {
			zeroCorrectedVal.RawBytes[j] = b
			if isZero[j] == 't' {
				zeroCorrectedVal.RawBytes[j] = byte(0)
			}
		}
		zeroCorrectedVal.ClearChecksum()
		zeroCorrectedVal.InitChecksum(key)

		for _, b := range zeroCorrectedVal.RawBytes {
			resp[respBookmark] = b
			respBookmark++
		}
...
func (txn *Txn) clearInsertHotkeys() {
	txn.insertHotkeys = make([][]byte, 0)
}
